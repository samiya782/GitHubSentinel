...
from hacker_news_client import HackerNewsClient  # 导入GitHub客户端类，处理GitHub API请求
from notifier import Notifier  # 导入通知器类，用于发送通知
from report_generator import ReportGenerator  # 导入报告生成器类
from llm import LLM, ReportType  # 导入语言模型类，可能用于生成报告内容
...
def hacker_news_job(hacker_news_client: HackerNewsClient | Type[HackerNewsClient], report_generator: ReportGenerator, notifier: Notifier, days: int, page: int = 1):
    LOG.info("[开始执行定时任务]")
    # 获取Hacker News的最新帖子
    markdown_file_path = hacker_news_client.export_daily_news(page)
    report_generator.generate_daily_report(markdown_file_path)
    notifier.notify('', markdown_file_path)
    LOG.info(f"[定时任务执行完毕]")
...
def main():
    ...
    notifier = Notifier(config.email, 'HackerNews')  # 创建通知器实例
    llm = LLM()  # 创建语言模型实例
    report_generator = ReportGenerator(llm)  # 创建报告生成器实例

    # 启动时立即执行（如不需要可注释）
    hacker_news_job(HackerNewsClient, report_generator, notifier, config.freq_days)

    # 安排每天的定时任务
    schedule.every(config.freq_days).days.at(
        config.exec_time
    ).do(HackerNewsClient, report_generator, notifier, config.freq_days)
