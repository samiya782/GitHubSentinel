...
from ah_gov_client import AhGovClient  # 导入安徽政府商机分析客户端
...
ah_gov_client = AhGovClient()  # 创建安徽政府商机分析客户端实例
...
def generate_ah_gov_report(model_type, model_name):
    ...
    markdown_file_path = ah_gov_client.export_articles()  # 导出安徽政府最新文章
    report, report_file_path = report_generator.generate_ah_gov_daily_report(markdown_file_path)  # 生成并获取报告内容及文件路径
    return report, report_file_path  # 返回报告内容和报告文件路径
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("GitHub 项目进展"):
        ...
        # 创建 Radio 组件
        model_type = gr.Radio(["gemini", "openai", "ollama"], value="gemini", label="模型类型", info="使用Gemini API、 OpenAI GPT API 或 Ollama 私有化模型服务")

        # 创建 Dropdown 组件
        # model_name = gr.Dropdown(choices=["gpt-4o", "gpt-4o-mini", "gpt-3.5-turbo"], value="gpt-4o-mini", label="选择模型")
        model_name = gr.Dropdown(choices=["gemini-2.5-flash", "gemini-2.5-pro"], value="gemini-2.5-flash", label="选择模型")
    ...
    # 创建安徽政府商机分析 Tab
    with gr.Tab("安徽政府商机分析"):
        gr.Markdown("## 安徽政府商机分析")  # 添加小标题
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_ah_gov_report, inputs=[model_type, model_name, ], outputs=[markdown_output, file_output])
